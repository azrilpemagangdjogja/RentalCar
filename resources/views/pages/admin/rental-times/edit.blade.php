            ...
            <div>
                <div class="mb-2 hidden sm:flex items-center gap-2 text-sm text-gray-400 dark:text-gray-400">
                    <a href="{{ route('rental-time.index') }}"
                        class="transition hover:text-gray-700 dark:hover:text-gray-200">Waktu Rental</a>
                ...
                <h1 class="text-2xl font-bold tracking-tight text-gray-800 dark:text-white sm:text-3xl">Edit Waktu Rental
                </h1>
                <p class="mt-1 hidden sm:block text-sm text-gray-500 dark:text-gray-400">Perbarui informasi waktu yang digunakan mitra pada
                    kendaraan miliknya.</p>
            </div>
        </div>

        {{-- FORM --}}

        <form action="{{ route('rental-time.update', $rentalTime->id) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="lg:col-span-2">
                    <div class="rounded-2xl bg-gray-50 dark:bg-gray-700">
                        <div class="border-b border-gray-200 p-5 dark:border-gray-600 sm:p-6">
                            ...
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Perbarui informasi dasar untuk waktu
                                rental yang digunakan pada sistem.</p>
                        </div>
                        ...
                        <div class="space-y-5 p-5 sm:p-6">
                            ...
                            <div>
                                ...
                                <input type="number" name="day" id="day"
                                    value="{{ old('name', $rentalTime->day) }}" placeholder="Contoh: 1, 2, 3, 7, 14"
                                    required
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="7" placeholder="Jelaskan wilayah atau cakupan region ini..."
                                    class="w-full resize-none rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm leading-6 text-gray-800 outline-none transition focus:border-gray-500 focus:ring-2 focus:ring-gray-800/10 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:placeholder-gray-500 dark:focus:border-gray-300 dark:focus:ring-white/10">{{ old('description', $rentalTime->description) }}</textarea>
                                @error('description')
                                    <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                    ...
                    <section
                        ...
                        <div class="mt-5">
                            ...
                            <select name="status" id="status"
                                ...
                                <option value="Inactive" @selected($rentalTime->status == 'Inactive')>Tidak Aktif</option>
                                <option value="Active" @selected($rentalTime->status == 'Active')>Aktif</option>
                            </select>
                        </div>
                    </section>
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-xs leading-5 text-gray-500 dark:text-gray-400">
                                    Pastikan nama region tetap jelas dan mudah dikenali pelanggan, terutama jika digunakan
                                    sebagai filter pencarian kendaraan.
                                </p>
                            </div>
                        </div>
                    </div>
